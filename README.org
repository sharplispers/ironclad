...
at [[https://github.com/froydnj/ironclad/archive/v0.34.tar.gz]].
If you are feeling adventurous, you can download a bleeding-edge version
at [[https://github.com/froydnj/ironclad]].

It comes with an ASDF system definition, so ~(asdf:oos 'asdf:load-op :ironclad)~
should be all that you need to get started. The testsuite can be run
by substituting ~asdf:test-op~ for ~asdf:load-op~ in the form above.
...
Instances of HMACs are constructed by specifying a secret key and
a digest-name.
...
Return an HMAC instance based on the hash function /digest-name/ with
secret key /key/.
...
The /:key/ argument is the secret key, as provided to [[make-hmac][make-hmac]].
...
Update the internal state of /hmac/ with the data in /sequence/
bounded by /start/ and /end/. /sequence/ must be a
...
Returns the MAC (/digest/) computed by /hmac/ thus far. The internal
state of /hmac/ is not modified; this feature makes it possible to
compute a "rolling MAC" of a document. The length of /digest/ is
determined by the [[digest-length][digest-length]] of the /digest-name/ passed to
[[make-hmac][make-hmac]] when /hmac/ was constructed.

If /buffer/ is provided, the computed MAC will be placed into
/buffer/ starting at /buffer-start/. /buffer/ must be a
~(simple-array (unsigned-byte 8) (*))~. An [[insufficient-buffer-space][insufficient-buffer-space]]
error will be signaled if there is insufficient space in /buffer/.

** CMAC

Instances of CMACs are constructed by specifying a secret key and
a cipher-name.
...
Return a CMAC instance based on the cipher /cipher-name/ with secret
key /key/. /cipher-name/ must have a [[block-length][block-length]] of either 8 or 16;
this restriction is satisfied by many ciphers in Ironclad with the
notable exception of stream ciphers. /key/ must be an acceptable key
for /cipher-name/.
...
Update the internal state of /cmac/ with the data in /sequence/
bounded by /start/ and /end/. /sequence/ must be a
~(simple-array (unsigned-byte 8) (*))~.
...
Returns the MAC (/digest/) computed by /cmac/ thus far. The internal
state of /cmac/ is not modified; this feature makes it possible to
compute a "rolling MAC" of a document. The length of /digest/ is
determined by the [[block-length][block-length]] of the /cipher-name/ passed to
[[make-cmac][make-cmac]] when /cmac/ was constructed.

** Blake2 and Blake2s MAC

Instances of Blake2 and Blake2s MACs are constructed by specifying
a secret key and optionally a digest-length.


#+NAME: make-blake2-mac
#+BEGIN_SRC lisp
(make-blake2-mac key &key digest-length) => mac
(make-blake2s-mac key &key digest-length) => mac
#+END_SRC

Return a Blake2 or Blake2s MAC instance with secret key /key/.
/digest-length/ can be any length from 1 to 64 for Blake2, or from
1 to 32 for Blake2s. By default, /digest-length/ is 64 for Blake2 and
32 for Blake2s.


#+NAME: update-blake2-mac
#+BEGIN_SRC lisp
(update-blake2-mac mac sequence &key start end) => mac
(update-blake2s-mac mac sequence &key start end) => mac
#+END_SRC

Update the internal state of /mac/ with the data in /sequence/ bounded
by /start/ and /end/. /sequence/ must be a
~(simple-array (unsigned-byte 8) (*))~.


#+NAME: blake2-mac-digest
#+BEGIN_SRC lisp
(blake2-mac-digest mac) => digest
(blake2s-mac-digest mac) => digest
#+END_SRC

Returns the MAC (/digest/) computed by /mac/ thus far. The internal
state of /mac/ is not modified; this feature makes it possible to
compute a "rolling MAC" of a document. The length of /digest/ is
determined by the /digest-length/ key parameter passed to
[[make-blake2-mac][make-blake2-mac]] or [[make-blake2-mac][make-blake2s-mac]] when /mac/ was constructed.

** Poly1305 MAC

Instances of Poly1305 MACs are constructed by specifying
a secret key.


#+NAME: make-poly1305
#+BEGIN_SRC lisp
(make-poly1305 key) => mac
#+END_SRC

Return a Poly1305 MAC instance with secret key /key/.
The size of /key/ must be 32 bytes.


#+NAME: update-poly1305
#+BEGIN_SRC lisp
(update-poly1305 mac data &key start end) => (values)
#+END_SRC

Update the internal state of /mac/ with the /data/ bounded by /start/
and /end/. /data/ must be a ~(simple-array (unsigned-byte 8) (*))~.


#+NAME: poly1305-digest
#+BEGIN_SRC lisp
(poly1305-digest mac) => digest
#+END_SRC

Returns the MAC (/digest/) computed by /mac/ thus far. The internal
state of /mac/ is not modified; this feature makes it possible to
compute a "rolling MAC" of a document. The length of /digest/ is 16
bytes.

** Skein MAC

Instances of Skein MACs are constructed by specifying a secret key and
optionally a block length and a digest-length.


#+NAME: make-skein-mac
#+BEGIN_SRC lisp
(make-skein-mac key &key block-length digest-length) => mac
#+END_SRC

Return a Skein MAC instance with secret key /key/. /block-length/ can
be 32 (to use the Skein256 hash function internally), 64 (to use
Skein512) or 128 (to use Skein1024). /digest-length/ can be any length
you want the computed MAC (/digest/) to be. By default, /block-length/
is 64 and /digest-length/ is 64.


#+NAME: update-skein-mac
#+BEGIN_SRC lisp
(update-skein-mac mac sequence &key start end) => mac
#+END_SRC
...
#+NAME: skein-mac-digest
#+BEGIN_SRC lisp
(skein-mac-digest mac) => digest
#+END_SRC
...
compute a "rolling MAC" of a document. The length of /digest/ is
determined by the /digest-length/ key parameter passed to
[[make-skein-mac][make-skein-mac]] when /mac/ was constructed.
...
Colin Percival's [[http://www.tarsnap.com/scrypt.html][The scrypt key derivation function]].
...
~(simple-array (unsigned-byte 8) (*))~), and number of digest iterations,
...
#+NAME: make-kdf
#+BEGIN_SRC lisp
(make-kdf kind &key digest n r p) => kdf
#+END_SRC

Returns a key derivation function instance (/kind/ must either be
/PBKDF1/, /PBKDF2/ or /SCRYPT-KDF/). The PBKDF algorithms use
/digest/. The scrypt key derivation uses cost parameters /N/, /r/ and
/p/ (/N/ is a CPU cost parameter that must be a power of 2, /r/ and
/p/ are memory cost parameters that must be defined such that
/r/ * /p/ <= 2^30).

The default parameters are /N/ = 4096, /r/ = 8, and /p/ = 2.
...
#+BEGIN_SRC lisp
(decrypt-message key message &key start end &allow-other-keys) => decrypted-message
#+END_SRC
...
the first half of the vector is the C1 value, the second half is the
C2 value). You can use the [[make-message][make-message]] and [[destructure-message][destructure-message]]
functions if you need access to the elements of a message (e.g. to use
a different kind of serialization).
...
according to the algorithm /digest/. [[produce-digest][produce-digest]] may be used to
obtain a digest of all the data written to the stream.
