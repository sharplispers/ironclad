...
(declaim (ftype (function ((unsigned-byte 32)) (unsigned-byte 32)) seed-g)
         (inline seed-g))
(defun seed-g (x)
...
(declaim (ftype (function ((unsigned-byte 32) (unsigned-byte 32) (unsigned-byte 32) (unsigned-byte 32)) (values (unsigned-byte 32) (unsigned-byte 32))) seed-f)
         (inline seed-f))
(defun seed-f (r0 r1 k0 k1)
  (declare (type (unsigned-byte 32) r0 r1 k0 k1)
           (optimize (speed 3) (space 0) (safety 0) (debug 0)))
  (let* ((a (logxor r0 k0))
         ...
         (s1 (seed-g (mod32+ c b)))
         (s0 (mod32+ s1 c)))
    (declare (type (unsigned-byte 32) a b c s0 s1))
    (values s0 s1)))
  ...
  (let ((round-keys (round-keys context))
    ...
    (with-words ((l0 l1 r0 r1) plaintext plaintext-start :size 4)
      ...
      (multiple-value-setq (t0 t1)
        (seed-f r0 r1 (aref round-keys 30) (aref round-keys 31)))
    ...
    (with-words ((l0 l1 r0 r1) ciphertext ciphertext-start :size 4)
      (loop for i from 15 downto 1 do
        ...
        (setf r0 (logxor r0 l0)
              ...
              l1 t1))
      (multiple-value-setq (t0 t1)
        (seed-f r0 r1 (aref round-keys 0) (aref round-keys 1)))
      (setf l0 (logxor l0 t0)
            l1 (logxor l1 t1))
