...
(define-block-encryptor camellia 16
  (let ((round-keys (round-keys context))
    ...
    (declare (type (simple-array (unsigned-byte 32) (*)) round-keys data)
             ...
             (dynamic-extent data))
    (dotimes (i 4)
      (setf (aref data i) (logxor (ub32ref/be plaintext (+ plaintext-start (* 4 i)))
                                  (aref round-keys i))))
    ...
    (dotimes (i 4)
      (setf (ub32ref/be ciphertext (+ ciphertext-start (* 4 i)))
            (logxor (aref data i) (aref round-keys (+ keys-start i)))))))
...
(define-block-decryptor camellia 16
  (let* ((round-keys (round-keys context))
    ...
    (declare (type (simple-array (unsigned-byte 32) (*)) round-keys data)
             ...
             (dynamic-extent data))
    (dotimes (i 4)
      (setf (aref data i) (logxor (ub32ref/be ciphertext (+ ciphertext-start (* 4 i)))
                                  (aref round-keys (+ keys-start i)))))
    (decf keys-start 2)
    ...
    (rotatef (aref data 1) (aref data 3))
    (dotimes (i 4)
      (setf (ub32ref/be plaintext (+ plaintext-start (* 4 i)))
            (logxor (aref data i) (aref round-keys (+ keys-start i)))))))
