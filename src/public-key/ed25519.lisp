...
(defun ed25519-decode-point (octets)
  (let* ((y (ed25519-decode-int octets))
         b x p)
    (setf b (ldb (byte 1 (- +ed25519-bits+ 1)) y))
    (setf (ldb (byte 1 (- +ed25519-bits+ 1)) y) 0)
    (setf x (ed25519-recover-x y))
...
(defun ed25519-sign (m sk pk)
  (let ((h (ed25519-hash sk))
    ...
    (setf (ldb (byte 3 0) (elt h 0)) 0)
    (setf (ldb (byte 2 6) (elt h (- (/ +ed25519-bits+ 8) 1))) 1)
    (setf a (ed25519-decode-int (subseq h 0 (/ +ed25519-bits+ 8))))
    (setf ri (ed25519-decode-int (ed25519-hash (subseq h (/ +ed25519-bits+ 8) (/ +ed25519-bits+ 4)) m)))
    (setf r (ed25519-scalar-mult +ed25519-b+ ri))
    (setf s (mod (+ (* (ed25519-decode-int (ed25519-hash (ed25519-encode-point r) pk m))
                       a)
                    ri)
                 +ed25519-l+))
