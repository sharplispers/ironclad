...
(defmethod ec-double ((p secp256k1-point))
  ...
  (with-slots ((x1 x) (y1 y) (z1 z)) p
    ...
    (if (zerop z1)
        ...
        (let* ((xx (mod (* x1 x1) +secp256k1-p+))
               (yy (mod (* y1 y1) +secp256k1-p+))
               (yyyy (mod (* yy yy) +secp256k1-p+))
               (zz (mod (* z1 z1) +secp256k1-p+))
               (x1+yy (mod (+ x1 yy) +secp256k1-p+))
               (y1+z1 (mod (+ y1 z1) +secp256k1-p+))
               (s (mod (* 2 (- (* x1+yy x1+yy) xx yyyy)) +secp256k1-p+))
               (m (mod (* 3 xx) +secp256k1-p+))
               (u (mod (- (* m m) (* 2 s)) +secp256k1-p+))
               (x2 u)
               (y2 (mod (- (* m (- s u)) (* 8 yyyy)) +secp256k1-p+))
               (z2 (mod (- (* y1+z1 y1+z1) yy zz) +secp256k1-p+)))
...
(declaim (notinline secp256k1-generate-k))
